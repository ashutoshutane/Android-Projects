...
class MainRepository(
    ...
    suspend fun getMovieCategories(apiKey: String): List<Category> {

        val trending = api.getTrendingMovies(apiKey).results
        val popular = api.getPopularMovies(apiKey).results
        val new = api.getNewMovies(apiKey).results
    ...
    suspend fun getSeriesCategories(apiKey: String): List<Category> {

        val trending = api.getTrendingSeries(apiKey).results
        val popular = api.getPopularSeries(apiKey).results
        val new = api.getNewSeries(apiKey).results
    ...
    suspend fun search(
        query: String,
        page: Int,
    ) = api.searchMulti(Constants.API_KEY,query,page)
