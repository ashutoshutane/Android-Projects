...
class HomeViewModel(
    ...
    private val _searchResults = MutableLiveData<List<ContentUI>>()
    val searchResults: LiveData<List<ContentUI>> = _searchResults
    ...
    private val loadedSearchItems = mutableListOf<ContentUI>()
    ...
    fun loadMovie() {
        ...
        viewModelScope.launch {
            ...
            try {
                _movies.value =
                    repo.getMovieCategories(Constants.API_KEY)
    ...
    fun loadSeries() {
        ...
        viewModelScope.launch {
            ...
            try {
                _series.value =
                    repo.getSeriesCategories(Constants.API_KEY)
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                nextItems.forEach { item ->
                    ...
                    val exists = loadedSearchItems.any {
                        it.id == item.id &&
                                it.type == item.type
                    }

                    if (!exists) {
                        loadedSearchItems.add(item)
                    }
                }

                _searchResults.value =
                    loadedSearchItems.toList()
